class AssignmentPdf < Prawn::Document
  ...
  def print_test_plan_results
    if (@assignment.test_plan.present?)
      ...
      @assignment.stencil.stencil_test_plans.each {
        |stencil_test_plan|
        if (!first)
          start_new_page
        else
        ...
        move_down 10
        cases = [["Name", "Category", "Version", "Description", "Result", "Date"]] +
        @assignment.results.select { |result| result.test_case.plan_cases.select { |plan_case| plan_case.test_plan_id == stencil_test_plan.test_plan.id}.present? }.sort.map { |result|
        ...
        move_down 20
        @assignment.results.select { |result| result.test_case.plan_cases.select { |plan_case| plan_case.test_plan_id == stencil_test_plan.test_plan.id}.present? && result.result != 'Passed' }.sort.map { |result|
